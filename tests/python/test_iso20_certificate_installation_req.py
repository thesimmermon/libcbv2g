...
lib.iso20_certificate_installation_req_encode_json_to_exi.argtypes = [ctypes.c_char_p, ctypes.POINTER(ctypes.POINTER(ctypes.c_uint8)), ctypes.POINTER(ctypes.c_size_t)]
lib.iso20_certificate_installation_req_encode_json_to_exi.restype = ctypes.c_int

lib.iso20_certificate_installation_req_decode_exi_to_json.argtypes = [ctypes.POINTER(ctypes.c_uint8), ctypes.c_size_t, ctypes.POINTER(ctypes.c_char_p)]
lib.iso20_certificate_installation_req_decode_exi_to_json.restype = ctypes.c_int
...
def test_iso20_certificate_installation_req():
    # Create test data based on CertificateInstallationReq-20.xml
    test_data = {
        "SessionID": "1234567890ABCDEF",
        "TimeStamp": "1677687277",
        "OEMProvisioningCertificateChain": {
            "Id": "ID1",
            "Certificate": "",
            "SubCertificates": [
                {
                    "Certificate": ""
                }
            ]
        },
    ...
    result = lib.iso20_certificate_installation_req_encode_json_to_exi(json_str, exi_buffer_ptr, ctypes.byref(exi_size))
    if result != 0:
    ...
    result = lib.iso20_certificate_installation_req_decode_exi_to_json(exi_buffer, exi_size, ctypes.byref(json_str_out))
    if result != 0:
...
if __name__ == "__main__":
    if test_iso20_certificate_installation_req():
        sys.exit(0)
    else:
        sys.exit(1) 